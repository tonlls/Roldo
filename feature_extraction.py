...
for t in TYPE:
    ...
    df.to_excel(basepath1+'Unlabeled audio/'+t+'/features_extracted.xlsx', index=False, header=True)
    df.to_json(basepath1+'Unlabeled audio/'+t+'/features_extracted.json')


    df = pd.read_excel(basepath1+'Unlabeled audio/'+t+'/features_extracted.xlsx')
    ...
    '''
TYPE=['Positive_audios','Negative_audios']
import soundfile as sf
for t in TYPE:

    basepath2 = basepath1+'Labeled audio/'+t+'/'

    original_len = len(os.listdir(basepath2))
    for i in os.listdir(basepath2):

        wav_file_path = os.path.join(basepath2, i)
    ...
    remove_len = len(os.listdir(basepath2))
    print('In total, {} audios have been removed due to short duration'.format(original_len-remove_len))


    # Let's extract the features

    [mid_term_features_pos, wav_file_list_pos, mid_feature_names] =  mF.directory_feature_extraction(basepath2, 0.5,0.5, 0.05, 0.05, compute_beat=False)
